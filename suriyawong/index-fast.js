...
(() => {
  ...
  function isAdminMode() {
    try { return Boolean(sessionStorage.getItem('LP360:TAMBOL:SURIYAWONG:mysiteAdminToken')); }
  ...
  function storageRead(storage, key, maxAgeMs) {
    ...
    try {
      const saved = JSON.parse(storage.getItem('LP360:TAMBOL:SURIYAWONG:SITE_FAST:' + key) || 'null');
  ...
  function writeCache(key, data) {
    ...
    try { sessionStorage.setItem('LP360:TAMBOL:SURIYAWONG:SITE_FAST:' + key, payload); } catch (_) {}
    try { localStorage.setItem('LP360:TAMBOL:SURIYAWONG:SITE_FAST:' + key, payload); } catch (_) {}
  }
    ...
    [window.sessionStorage, window.localStorage].forEach(storage => {
      try {
        Object.keys(storage).forEach(key => {
          if (!key.startsWith('LP360:TAMBOL:SURIYAWONG:SITE_FAST:')) return;
...
(() => {
  ...
  const STORAGE_CACHE_KEY='LP360:TAMBOL:SURIYAWONG:mysiteAdminStorageV2D15';
  ...
  function loadAdminStorage(force=false){
    const token=sessionStorage.getItem('LP360:TAMBOL:SURIYAWONG:mysiteAdminToken');
  ...
  $('adminLogoutButton').addEventListener('click',()=>{sessionStorage.removeItem('LP360:TAMBOL:SURIYAWONG:mysiteAdminToken');sessionStorage.removeItem('LP360:TAMBOL:SURIYAWONG:mysiteAdminName');setAdminUi(false);if(window.Swal)Swal.close()});
  const existingToken=sessionStorage.getItem('LP360:TAMBOL:SURIYAWONG:mysiteAdminToken');
  if(existingToken){
    api({mode:'editwebsite',editor:'text',token:existingToken})
      .then(()=>loadAdminTools())
      .then(()=>{setAdminUi(true);loadAdminStorage(false)})
      .catch(()=>{sessionStorage.removeItem('LP360:TAMBOL:SURIYAWONG:mysiteAdminToken');sessionStorage.removeItem('LP360:TAMBOL:SURIYAWONG:mysiteAdminName');setAdminUi(false)});
...
(() => {
  ...
  async function searchAndOpenProfile(rollno) {
    ...
    try {
      ...
      try { sessionStorage.setItem('LP360:TAMBOL:SURIYAWONG:SSS_PROFILE_ROLLNO', rollno); } catch (_) {}
    } catch (error) {
...
(() => {
  ...
  const CACHE_KEY = 'LP360:TAMBOL:SURIYAWONG:studentServiceTop3:v7-top3-heading-org';
...
(() => {
  ...
  const EXEC_CACHE_KEY = 'LP360:TAMBOL:SURIYAWONG:SITE_FAST:cliproom-exec-v3';
  const EXEC_CACHE_AGE = 10 * 60 * 1000;
  const CATALOG_CACHE_KEY = 'LP360:TAMBOL:SURIYAWONG:SITE_FAST:cliproom-catalog-v3-dynamic-exec';
...
(() => {
  ...
  let student = JSON.parse(localStorage.getItem('LP360:TAMBOL:SURIYAWONG:LEARN_STUDENT') || 'null');
...
if (!fullname || !phone) {
  ...
  try {
    ...
    localStorage.setItem(
      'LP360:TAMBOL:SURIYAWONG:LEARN_STUDENT',
  ...
  async function registerStudent() {
    ...
    try {
      ...
      student = res.student;
      localStorage.setItem('LP360:TAMBOL:SURIYAWONG:LEARN_STUDENT', JSON.stringify(student));
...
async function loadMyTotalHours() {
  ...
  function logoutFromEditProfile() {
    Swal.fire({
      ...
      student=null;
      localStorage.removeItem('LP360:TAMBOL:SURIYAWONG:LEARN_STUDENT');
      closeEditProfile();
      closeModal('studentModal');
      updateTop();
      const root=$('learningBaseModule');
  ...
  function closeStudentModal() {
    Swal.fire({title:'ออกจากระบบ?',icon:'warning',showCancelButton:true,confirmButtonText:'ออกจากระบบ',cancelButtonText:'ยกเลิก'}).then(r=>{if(!r.isConfirmed)return;student=null;localStorage.removeItem('LP360:TAMBOL:SURIYAWONG:LEARN_STUDENT');closeModal('studentModal');updateTop();showPage('activitiesPage',$('learningBaseModule').querySelector('.learning-tabs button'));});
  }
...
(() => {
  ...
  function getStudent() {
    try {
      return JSON.parse(
        localStorage.getItem('LP360:TAMBOL:SURIYAWONG:LEARN_STUDENT') || 'null'
      );
    } catch (_) {
      return null;
    }
  ...
  window.addEventListener('storage', event => {
    if (event.key === 'LP360:TAMBOL:SURIYAWONG:LEARN_STUDENT') {
      renderProfile();
    }
  });
